...
- [alice.z@leetcode.com, alicez@leetcode.com] forward to the same address
- Everything after first plus sign will be ignored
- only 1 '@'
- How many different addresses receive mails?
...
if "@" in email:
    ...
    sys.stderr("Invalid input")
    sys.exit(1)

if "+" in name:
    tag_index = domain.index("+")
    name = name[:tag_index]
...
class Solution:
    def numUniqueEmails(self, emails):
        ...
        for email in emails:
            ...
            name = "".join([ch for ch in name if ch != "."]) + "@" + domain
