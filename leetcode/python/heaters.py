...
This one was a real PITA. Why? Because I struggled first with thinking of a
solution, then after a hint, doing a modified binary search.
        ...
        for house in houses:
            ...
            while r >= l:
                ...
                if house == heaters[m]:
                    ...
                    dist = min(dist, heaters[m] - house)
                    r = m - 1
                elif house > heaters[m]:
                    dist = min(dist, house - heaters[m])
                    l = m + 1

            min_radius = max(dist, min_radius)
